...
user_seed = 0x909324FC 
user_pointer = 0x0227116C
user_tid = 0x8888
user_sid = 0x422a
user_lang = "en"
...
LANGUAGES = {"jp", "en", "fr", "it", "ge", "_6", "sp", "ko", "_end"}

RNG_LOC = {0, 0x021BFB14, 0x021BFCF4, 0, 0, 0, 0, 0} -- en fr
BPT_LOC = {0, 0x02101D2C, 0x02101F0C, 0, 0, 0, 0, 0} -- en fr
WILD_OFF = 0x58E3C
OG_WILD_OFF = 0x3740C
...
function GetRng()
	rng = memory.readdword(rng_index)
  return rng
end

function SetRng()
	memory.writedword(rng_index, bit.band(user_seed))
	print("Seed is now "..string.format("%.8X", user_seed))
end

function RngBackwards(f)
  for i=1, f, 1 do
    state = memory.readdword(rng_index)
    state = Mult32(state, 0xEEB9EB65) + 0xA3561A1
    memory.writedword(rng_index, bit.band(state))
  end
end
...
function GetPointer()
	return memory.readdword(bpt_index)
end
...
function Main()
  rng_index = RNG_LOC[GetLanguageIndex(user_lang)]
  bpt_index = BPT_LOC[GetLanguageIndex(user_lang)]
  pointer = GetPointer()
